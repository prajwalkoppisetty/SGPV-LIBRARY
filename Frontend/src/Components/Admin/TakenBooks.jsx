...
export default function TakenBooks() {
  ...
  // Fetch all orders with status 'recieved' (books with students) or by Order_ID if searched
  const fetchOrders = async (orderId = '') => {
    ...
    try {
      ...
      if (orderId) {
        ...
        setOrders(res.data.order && res.data.order.Status === 'recieved' ? [res.data.order] : []);
      } else {
        res = await axios.get('http://localhost:3000/api/auth/admin/orders/recieved', { withCredentials: true });
  ...
  // Handle "Book Returned" action
  const handleReturned = async (orderId) => {
    setActionLoading(true);
    setActionMessage('');
    try {
      await axios.post(
        `http://localhost:3000/api/auth/admin/orders/${orderId}/returned`,
        {},
        { withCredentials: true }
      );
      setActionMessage('Book marked as returned and thank you email sent.');
      fetchOrders();
    } catch {
  ...
  return (
    <div className="w-full max-w-xs p-2 sm:max-w-md md:max-w-3xl md:p-4 mx-auto mt-10 bg-white rounded-xl shadow">
      ...
      <h2 className="text-2xl font-bold mb-6 text-blue-700 text-center">Taken Books</h2>
      {actionMessage && (
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-500">No books currently with students.</div>
      ) : (
        orders.map(order => (
          <div key={order._id} className="mb-6 border-b pb-4">
            ...
            <div>
              ...
              <span className="ml-2 text-red-600">{order.Return_Date}</span>
            </div>
            <div className="mt-2">
            ...
            <div className="mt-4 flex gap-3">
              <button
                ...
                onClick={() => handleReturned(order._id)}
